...
DEFINE_int32(raft_rpc_channel_connect_timeout_ms, 1000,
             "max connect time for rpc");
    ...
    if (_state == STATE_LEADER) {
        ...
        if (_conf.is_singleton()) {
            ...
            if (read_type == READ_ONLY_TYPE_SAFE) {
                ...
                Ballot ballot;
                ballot.init(_conf.conf, _conf.stable() ? NULL : &(_conf.old_conf));
...
bool NodeImpl::validate_configuration(const Configuration& old_conf,
    ...
    //    a learner node to a REPLICA node or vice versa.
